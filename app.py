...
def chat():
    ...
    # Conversation identity for persistence.
    # The conversation_id is generated by the client and sent with every message
    conversation_id = body.get("conversation_id") or uuid.uuid4().hex
    # turn_index positions each message within the conversation.
    user_turn = len(messages) - 1
    assistant_turn = len(messages)
    ...
    def generate():
        # Let the client learn (and reuse) the conversation id, and tell it
        # whether this conversation is actually being stored server-side.
        yield sse_event("meta", {
            "conversation_id": conversation_id,
            # Also, let the JS client know whether storage is enabled
            # (we can use it for example to display a "send feedback" button)
            "stored": storage.is_enabled(),
        })

        # Persist the conversation and the incoming user message. No-ops when
        # no storage backend is installed; never breaks the chat on failure.
        now = datetime.now(timezone.utc).isoformat()
        storage.ensure_conversation(conversation_id, metadata={"model": settings.AI_MODEL})
        if user_message is not None:
            storage.save_message(
                conversation_id, user_turn,
                user_message.get("role", "user"),
                user_message.get("content", ""),
                now,
            )
        tool_seq = 0  # running index of tool calls within this assistant turn
        ...
        for round_idx in range(MAX_TOOL_ROUNDS):
            ...
            if not tool_calls:
                ...
                storage.save_message(
                    conversation_id, assistant_turn, "assistant", reply,
                    datetime.now(timezone.utc).isoformat(),
                )
                yield sse_event("result", {"reply": reply})
                return
            ...
            for tc in tool_calls:
                ...
                storage.save_tool_call(
                    conversation_id, assistant_turn, tool_seq,
